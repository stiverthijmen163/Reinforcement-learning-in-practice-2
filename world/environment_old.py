...
class Environment:
    ...
    def evaluate_agent(grid_fp: Path,
        ...
        Args:
            ...
            reward_fn: callable reward function returning a reward value.
            show_images: Whether to show the images at the end of the
                evaluation. If False, only saves the images.
            save_path: Path to save evaluation outputs at the end of the evaluation.
            save_image: Whether to save the path image PNG.
        """
        ...
        file_name = datetime.now().strftime("%Y-%m-%d__%H-%M-%S-%f")[:-3] if not save_name else save_name# Milliseconds precision to avoid overwriting files

        save_results(file_name, env.world_stats, path_image, show_images, save_path, save_image)
