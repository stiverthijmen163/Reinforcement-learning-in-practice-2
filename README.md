...
```bash
python train_value_iteration.py grid_configs/A1_grid.npy --no_gui --sigma 0.02 --iter 200 --gamma 0.9 --theta 0.1 --reward default
```

### Policy Iteration

```bash
python train_policy_iter.py grid_configs/A1_grid.npy --no_gui --sigma 0.02 --iter 200 --gamma 0.9 --theta 0.01 --reward policy
```

### On-Policy Monte Carlo

```bash
python train_mc.py grid_configs/A1_grid.npy --no_gui --sigma 0.02 --iter 200 --episodes 1000 --epsilon 0.1 --gamma 0.9 --reward mc
```

### Off-Policy Monte Carlo

```bash
python train_off_policy_mc.py grid_configs/A1_grid.npy --no_gui --sigma 0.02 --iter 200 --episodes 1000 --epsilon 0.1 --gamma 0.9 --reward mc
```

### SARSA

```bash
python train_sarsa.py grid_configs/A1_grid.npy --no_gui --sigma 0.02 --iter 200 --episodes 1000 --epsilon 0.1 --gamma 0.9 --alpha 0.5 --reward default
```

### Q-Learning

```bash
python train_qlearning.py grid_configs/A1_grid.npy --no_gui --sigma 0.02 --iter 200 --episodes 1000 --epsilon 0.1 --gamma 0.9 --alpha 0.5 --reward default
```

### Run all configured experiments

Set the grids/agents/hyperparameters in `evaluation/config.py`, then run:

```bash
python -m evaluation.run_experiments
```

Results are saved in `results/evaluation_results/`.

### Create plots based on evaluation results

Creates bar plots analyzing the impact of each hyperparameter on success rate
and cumulative reward, broken down per agent and reward function.

```bash
python -m evaluation.analyze_results results/evaluation_results/TIMESTAMP
```

Results are saved in `results/evaluation_results/TIMESTAMP/analysis/`, with one
subdirectory per grid (e.g. `analysis/A1_grid/`, `analysis/small_grid/`) and one
plot per hyperparameter inside each.

Note that TIMESTAMP corresponds to a evaluation_results folder